...
class MainActivity : AppCompatActivity() {
    ...
    private fun readFile(fileName: String): String {
        ...
        if (fileName === "madlibs1") {
            val scan: Scanner = Scanner(getResources().openRawResource(R.raw.madlib1_tarzan))
            text = scan.nextLine()
            while (scan.hasNextLine()) {
                text += " " + scan.nextLine()
            }
        } else {
            val scan: Scanner = Scanner(getResources().openRawResource(R.raw.madlib2_university))
            text = scan.nextLine()
            while (scan.hasNextLine()) {
                text += " " + scan.nextLine()
            }
